cc.Class({
  ...
  onCollisionExit: function() {
    ...
    if (PlantNodes) {
      //是否成熟并且选择是镰刀收割工具
      if (this.dataList.List[id].CropsStatus == 4 && this.dataList.toolType == 4) {
      ...
      //浇水
      if (this.dataList.List[id].IsDry && this.dataList.toolType == 1) {
        var action = cc.fadeOut(0.6);
        PlantNodesTip.runAction(action);
      }
      //除草
      if (this.dataList.List[id].IsWeeds && this.dataList.toolType == 2) {
        var action = cc.fadeOut(0.6);
        PlantNodesTip.runAction(action);
      }
      // //除虫
      // if (this.dataList.List[id].chuchong && this.dataList.toolType == 3) {
      //   var action = cc.fadeOut(0.6);
      //   PlantNodesTip.runAction(action);
      // }
    }
